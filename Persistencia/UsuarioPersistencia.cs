    ...
    public class UsuarioPersistencia
    {
        public Credencial Login(String nombreUsuario)
        {
            ...
            if (lstCredenciales.Count() > 0)                                                                                                // (solo si encuentra reistro con nombreUsuario sino queda salida = null)
            {
                ...
                salida = new Credencial(strCredencial, intentos, bloqueo, usuarioPerfil, descripcionPerfil, roles);                                            //  Instancia la credencial con los datos obtenidos
            }
            return salida;
        }
        ...
        {
            List<Credencial> salida = new List<Credencial>();

            DataBaseUtils dataBaseUtils = new DataBaseUtils();

            List<String> lstCredenciales = dataBaseUtils.BuscarRegistrosPorValor("credenciales.csv", -1, "");
            if (lstCredenciales.Count() > 0)
            {
                foreach (String strCredencial in lstCredenciales)                                                                 //      Recorre la lista de personas
                {
                    String[] camposCredencial = strCredencial.Split(';');
                    ...
                    salida.Add(new Credencial(strCredencial, intentos, bloqueo, usuarioPerfil, descripcionPerfil, roles));
                }
            }
